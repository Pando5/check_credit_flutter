...
class _Detail_CreditState extends State<Detail_Credit> {
  ...
  Future<String> _callService() async {
    doc = await _apiService.detailCreditService(so_id, soDocNo);
    setState(() {
      // data = doc;
      data = doc[0];
    });
  ...
  Future callReason() async {
    List<Reason> data = [];
    data = await _apiService.reasonService();
    setState(() {
      reason = data;
    });
    print(reason);
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: FutureBuilder<String>(
        // กำหนดชนิดข้อมูล
        future: _callService(), // ข้อมูล Future
